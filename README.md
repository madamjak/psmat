...
```
Example `userconfig.json` file allows you to change language for editor info messages or configure "dark mode". Note that only language codes configured in `Config/Locale` will work.
...
### Syntax-highlight
Syntax highlight is configurable via `jazyk.json` file located in ```/Config``` folder in your installation location. Example simple config shown bellow. The file allows to configure multiple languages, and the appropriate is selected based on file extension. Config allows to configure symbols for the source code comments, strings and list of regular expressions to match programming language keywords or other token types supported by editor. 
...
## License
The project is unlicensed, see the [LICENSE](LICENSE) file for details. Anyone who may find this useful to educate themselves is free to either create own fork or suggest improvements in this repository directly.
